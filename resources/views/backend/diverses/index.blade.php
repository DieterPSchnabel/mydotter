...
// init each (key/value pair) one time with value = 1 (true) - checks if already exists - later use the switches
...
//$display_cols_str = get_dv_not_cached($this_table_name . '_disp_cols_arr', 'div_res_long');
$display_cols_str = get_dv($this_table_name . '_disp_cols_arr', 'div_res_long');
//dd($display_cols_str);
$display_cols_arr = explode(',', $display_cols_str);
$display_cols_arr = array_unique($display_cols_arr);
...
@section('content')
    ...
    @if(get_dv($this_table_name.'_table_display_debug_area'))
        <div style="padding:12px;background:#fffff6;border:1px #ccc solid">
            ...
            echo get_checkbox_any_table(
                ...
                $with_comment = false,
                $tt_hint_key = 'is_dev',
                $label_text = get_tr("Debug-Bereich anzeigen?"),
            ...
            <div id="debug_inner">
                ...
                //                ec(timeAgo(get_dv_not_cached($id, 'updated_at')));
                //                ec(timeAgo(get_dv($id, 'updated_at')));
                //ec(get_div_what_by_id(63051));
                //ec($display_cols_str,'','font-size:1.4em');
                //ec(get_dv_not_cached($this_table_name . '_disp_cols_arr', 'div_res_long'),'','font-size:1.4em');
                //$display_cols_str

                //            ec(get_default_lang_name());
                //            ec(get_default_lang_code());
                //
                //            ec('get_fallback_lang_name: '.get_fallback_lang_name());
                //            ec('get_fallback_lang_code: '.get_fallback_lang_code());
                ...
                /*echo active_languages_str_for_diverses_raw_query();
                echo '<br><br>';
                $t_key = 'is_dev';
    ...
    <div class="card">
        ...
        <div class="card-header" style="border-bottom:none">
            ...
            <div class="btn-toolbar float-right" role="toolbar" aria-label="Toolbar with button groups">
                ...
                @if(get_dv($this_table_name.'_table_has_help'))
                    {{------------ TABLE HILFE ------------}}
                    <a style="margin-left:12px" class="btn btn-info" href="javascript:toggle('help_block','slide')"
                       role="button">
                        <i class="icon-settings"></i> {!! get_tr('Hilfe') !!}</a>
                    <?php
      ...
      <b>{{$dataset->firstItem()}}</b> - <b>{{$dataset->lastItem()}}</b> / <b>{{$dataset->total()}}</b> {!! get_tr('Datensätze') !!}
    </span>

            @if($display_cols_str<>$sortables_str)
                <span style="margin-left:30px;font-size:0.9em;color:#bbb;font-weight:normal;">{!! get_tr('Einige Spalten sind ausgeblendet!') !!}</span>
            @endif
            ...
            @if(isset($_GET['mysearch']) or isset($_GET['myid']) or isset($_GET['filter']))
                @if( isset($_GET['mysearch']) )
                    <span style="margin-left:60px;font-size:0.9em;font-weight:normal"><b>{{$dataset->total()}}</b>
                        @if ($dataset->total()==1)
                            {!! get_tr('Ergebnis für Suche nach') !!}:
                        @else
                            {!! get_tr('Ergebnisse für Suche nach') !!}:
                        @endif
                ...
                <?php
                $filter_name = '';
                if (isset($_GET['filter'])) $filter_name = ' (<b>' . $_GET['filter'] . '</b>)';
                ?>

                <span style="margin-left:40px;font-size:0.6em;font-weight:normal">
            <a class="btn btn-outline-success btn-sm" href="{!! route('admin.'.$cc_table_name.'.index') !!}">
                {!! get_tr('Einige Records sind ausgefiltert') !!}{!! $filter_name !!},
                {!! get_tr('wieder alle anzeigen!') !!}</a></span>
            @endif
        </div>
        {{-- / END HEAD  1. ROW ------------}}
        ...
        <div style="background-color: #f0f3f5;padding-top:4px;padding-bottom:0;">
            <div class="input-group float-left" style="padding-bottom:0;margin-top:0px;">
                {{------------ SEARCH ------------}}
                @if($has_table_search)
                    <form id="mysearch_form" method="get" action="{{ route('admin.'.$cc_table_name.'.index') }}"
                          style="margin:0 50px 3px 18px;">
                        <div class="input-group">
                            @if( isset($_GET['mysearch']) )
                                <input style="" name="mysearch" id="mysearch" type="text" class="form-control "
                                       placeholder="{!! get_tr('Suche nach') !!}..." value="{{$_GET['mysearch']}}"
                                       autofocus>
                            @else
                                <input style="" name="mysearch" id="mysearch" type="text" class="form-control "
                                       placeholder="{!! get_tr('Suche nach') !!}...">
                            @endif
                            <span class="input-group-btn">
                        <button onClick='$("#mysearch_form").submit()' class="btn btn-default" type="button"
                                data-toggle="tooltip" data-placement="right" title=""
                                data-original-title="Suchtext eingeben und hier klicken oder Enter (auf der Tastatur)">
                        ...
                        <div class="dropdown">
                            ...
                            <div style="margin-top:37px" class="dropdown-menu animated slideInDown"
                                ...
                                <a class="dropdown-item"
                                   href="{{ URL::to('admin/'.$cc_table_name.'/export/pdf') }}">PDF</a>
                            </div>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="btn-group" style="margin:0 30px 3px 0;margin-bottom:-15px;">
                        <div class="dropdown">
                            ...
                            <div style="margin-top:37px" class="dropdown-menu animated slideInDown"
                                ...
                                <a class="dropdown-item"
                                   href="javascript:show_only_activated_langs('{{$this_table_name}}')">
                                    {!! get_tr('Zeige nur Sprachen welche <br>aktiviert sind. (Spalten-Filter)') !!}
                                </a>


                                <a class="dropdown-item"
                                   href="{!! route('admin.'.$this_table_name.'.index') !!}?filter=show_only_hints">

                                    {!! get_tr('Zeige nur Datensätze welche <br>übersetzt werden können. (Datensatz-Filter)') !!}
                                </a>

                                <a class="dropdown-item"
                                   href="{!! route('admin.'.$this_table_name.'.index') !!}?filter=show_only_missing_transl">
                                    {!! get_tr('Zeige nur Datensätze welche <br>übersetzt werden können<br>
                                    und wo Übersetzungen fehlen. (Datensatz-Filter)') !!}
                                </a>


                                <a class="dropdown-item"
                                   href="{!! route('admin.'.$this_table_name.'.index') !!}?filter=show_only_switches">
                                    {!! get_tr('Zeige nur logische Schalter.<br>(Datensatz-Filter)') !!}
                                </a>

                                <a class="dropdown-item" href="javascript:show_only_key_values('{{$this_table_name}}')">

                                    {!! get_tr('Zeige nur Key/Value Paare.<br>
                                    div_what, div_res, div_res_long<br>
                                    (Datensatz-Filter)') !!}
                                ...
                                <a class="dropdown-item" href="{!! route('admin.'.$cc_table_name.'.index') !!}">
                                    {!! get_tr('alle anzeigen') !!}
                                </a>

                                {!! link_to_route('admin.languages.index',get_tr('gehe zu') .' Languages',[],[ 'target="_blank"', 'class="dropdown-item dimmed04"']) !!}
                    ...
                    <div class="btn-group" style="margin:0 30px 0 0;margin-bottom:-15px;">
                        <div class="dropdown" style="">
                            ...
                            <div style="margin-top:37px" class="dropdown-menu animated slideInDown"
                                 aria-labelledby="dropdownMenuButton">
                                <div>
                                    <a href="javascript:display_all_cols('{{$this_table_name}}')">
                                        <button class="btn btn-link btn-sm">{!! get_tr('alle anzeigen') !!}</button>
                                    </a>
                                    <a href="javascript:hide_all_cols('{{$this_table_name}}')">
                                        <button class="btn btn-link btn-sm">{!! get_tr('alle ausblenden') !!}</button>
                                    </a>
                                    <?php
                                    $what = 'table_display_cols_hint';
                                    $class = "tip";
                                    ...
                                    echo tooltip($what, $class, $width, $style, $icon);
                                    ?>
                                    <span id="{{$this_table_name}}_display_all_conf"
                                          style="margin-left:6px;width:30px"></span>
                                </div>
                                <form id="tab_cols">
                            ...
                            background:#f9f9f9;border:none;">
                                        ...
                                        <button type="submit" class="btn btn-primary btn-sm">
                                            {!! get_tr('Speichern und Reload') !!}
                                        </button>
                                        <span id="{{$this_table_name}}_get_cols_conf"
                                              style="margin-left:6px;width:30px"></span>
                                    </div>
                                        ...
                                        <div class="card-content " style="width:200px">
                                            <ul style="list-style-type: none;margin:2px 3px;padding:0;line-height:240%;background:#f9f9f9">
                                                ...
                                                foreach ($cols_arr as $col) {
                                                    ...
                                                    if (in_array($col, $display_cols_arr)) {
                                                        $checked = 'checked';
                                                    }
                                ...
                                <div><a href="javascript:display_all_cols('{{$this_table_name}}')">
                                        <button class="btn btn-link btn-sm">
                                            {!! get_tr('alle anzeigen') !!}</button>
                                    </a>

                                    <a href="javascript:hide_all_cols('{{$this_table_name}}')">
                                        <button class="btn btn-link btn-sm">
                                            {!! get_tr('alle ausblenden') !!}</button>
                                    </a>
                                    <span id="{{$this_table_name}}_display_all_conf"
                                          style="margin-left:6px;width:30px"></span></div>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                @if($has_items_per_page)
                    ...
                    <div style="margin-left:30px;margin-bottom:-15px;margin-top:4px">
                        <span style="display:inline-block;margin:0 6px 3px 0;vertical-align:text-top;">{!! get_tr('Records pro Seite') !!}
                            :</span>
                ...
                {{------------ Open Languages ------------}}
                <div class="btn-group ml-30" style="margin:0 30px 3px 0;margin-bottom:-15px;">
                    <div class="dropdown">
                        <button class="btn btn-light dropdown-toggle" type="button" id="dropdownMenuButton"
                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            {!! get_tr('gehe zu') !!} Languages
                        </button>
                        <?php
        ...
        <div class="card-body" style="padding:0;border-top:none;overflow:auto">
            ...
            <div class="round8"
                 style="margin:-8px 7px 0 7px;height:53px;padding:8px 25px 8px 25px;background:#EFF2F4;opacity:1">
            ...
            <span class="dimmed04" style="margin-left:30px;">
             <b>{{$dataset->firstItem()}}</b> - <b>{{$dataset->lastItem()}}</b> / <b>{{$dataset->total()}}</b> records</span>
                ...
                @endif
                    ...
                    @if(isset($_GET['mysearch']) or isset($_GET['myid']) or isset($_GET['filter']) )
                        <span style="margin-left:40px;font-size:1.0em"><a class="btn btn-outline-success"
                                                                          href="{!! route('admin.'.$cc_table_name.'.index') !!}">
                                {!! get_tr('Einige Records sind ausgefiltert') !!} {!! $filter_name !!},
                                {!! get_tr('wieder alle anzeigen!') !!}</a></span>
                    @endif
                    {{--<a href="{{url()->previous()}}" class="btn btn-default ml-4 " role="button">
                        {!! get_tr('Abbruch und zurück') !!}</a>--}}
            </span>
            </div>
