...
@section('page-header')
    <div class="text-right dimmed04" style="margin:-6px 12px 6px 12px;color:#123;">
        Festlegen welche Sprachen im Backend und im Frontend verfügbar sind. Die Reihenfolge der Sprachen festlegen. Mehr in der Hilfe...
    </div>
@endsection
...
<div class="card-header" style="border-bottom:none">
    ...
    <div class="btn-toolbar float-right" role="toolbar" aria-label="Toolbar with button groups">
        ...
        <a class="btn btn-primary ml-1" role="button" href="{!! route('admin.'.$this_table_name.'.create') !!}">neu erfassen</a>
        @endif
    ...
    @if(isset($_GET['mysearch']) or isset($_GET['myid']) or isset($_GET['filter']))
        ...
        <a class="btn btn-outline-success" href="{!! route('admin.'.$this_table_name.'.index') !!}">Einige Records sind ausgefiltert, wieder alle Daten anzeigen!</a></span>
    @endif
</div>
{{-- / END HEAD  1. ROW ------------}}
...
<div style="background-color: #f0f3f5;padding-top:4px;padding-bottom:0;">
    <div class="input-group float-left" style="padding-bottom:0;margin-top:0px;">
        {{------------ SEARCH ------------}}
        @if($has_table_search)
            <form id="mysearch_form" method="get" action="{{ route('admin.'.$this_table_name.'.index') }}" style="margin:0 50px 3px 18px;">
        ...
        @if($has_table_export)
            <div class="btn-group" style="margin:0 30px 3px 0px;margin-bottom:-15px;">
                <div class="dropdown">
                    ...
                    echo tooltip($what,$class, $width,  $style, $icon);
                    ?>
                    <div style="margin-top:37px" class="dropdown-menu animated slideInLeft" aria-labelledby="dropdownMenuButton">
                        <a class="dropdown-item" href="{{ URL::to('admin/'.$this_table_name.'/export/xls') }}">Excel.xls</a>
                        <a class="dropdown-item" href="{{ URL::to('admin/'.$this_table_name.'/export/xlsx') }}">Excel.xlsx</a>
                        <a class="dropdown-item" href="{{ URL::to('admin/'.$this_table_name.'/export/csv') }}">Excel.csv</a>
                        <a class="dropdown-item" href="{{ URL::to('admin/'.$this_table_name.'/export/pdf') }}">PDF</a>
                    </div>
                </div>
            </div>
        @endif
            ...
            <div class="btn-group" style="margin:0 30px 3px 0;margin-bottom:-15px;">
                <div class="dropdown">
                    ...
                    <div style="margin-top:37px" class="dropdown-menu animated slideInDown" aria-labelledby="dropdownMenuButton">
                        <a class="dropdown-item" href="{!! route('admin.'.$this_table_name.'.index') !!}?filter=has_status">
                            nur status = true oder status_front_end = true</a>
                        <a class="dropdown-item" href="{!! route('admin.'.$this_table_name.'.index') !!}">alle zeigen</a>
                    </div>
                </div>
            </div>
        @endif
        ...
        {{--create your own filters in controller or deactivate display of filters completly with $has_cols_filter on top of this page --}}
        @if($has_cols_filter)
                        ...
                        <form id="tab_cols">
                            <div class="card" style="padding-bottom:0;margin-bottom:0;background:#fff">
                                <div class="card-content " style="width:200px">
                                    <ul style="list-style-type: none;margin:0;padding:5px 9px;line-height:240%;background:#f9f9f9">
                                        <?php
                                        $cols_arr = get_columns_from_table_sortable($this_table_name,$as_array=true);
                                        foreach ($cols_arr as $col) {
                                            echo '<li class="dimmed04" style="border-bottom:1px #ccc solid;background:#fff">';
                                            $checked='';
    ...
    <div class="card-body" style="padding:0;border-top:none;overflow:auto">
        ...
        <div class="round8" style="margin:-8px 7px 0 7px;height:53px;padding:8px 25px 8px 25px;background:#EFF2F4;opacity:1">
            ...
            <span class="float-left">
                @if($has_action_ceate_new)
                    <a class="btn btn-primary" role="button" href="{!! route('admin.'.$this_table_name.'.create') !!}">neu erfassen</a>
                @endif

                @if(isset($_GET['mysearch']) or isset($_GET['myid']) or isset($_GET['filter']) )
                    <span style="margin-left:40px;font-size:1.0em"><a class="btn btn-outline-success" href="{!! route('admin.'.$this_table_name.'.index') !!}">Einige Records sind ausgefiltert, wieder alle Daten anzeigen!</a></span>
                @endif
